...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group editable-card-shine block overflow-hidden rounded-md border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        ...
        <h2 className="mt-4 line-clamp-3 text-2xl font-black leading-tight tracking-[-0.04em] text-[var(--slot4-page-text)]">{post.title}</h2>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg)] text-[var(--editable-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-md border border-[var(--editable-border)] bg-[var(--slot4-dark-bg)] p-6 text-white shadow-[0_30px_90px_rgba(8,33,59,0.16)] md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#f2be7b]">{pagesContent.search.hero.badge}</p>
              <h1 className="mt-5 text-4xl font-extrabold leading-[1.02] tracking-[-0.04em] sm:text-6xl">{pagesContent.search.hero.title}</h1>
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 text-white/78">{pagesContent.search.hero.description}</p>
            </div>
            <form action="/search" className="self-end rounded-md border border-white/15 bg-white p-4 text-[var(--slot4-page-text)] shadow-xl sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-md bg-[var(--slot4-accent)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
